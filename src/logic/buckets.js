...
export const getDefault = _ => DefaultPreference.get('label-focus')
...
export const getCounters = async _ => {
...
export const update = async (data, bucket, date, change) =>
	change > 0 ? await _add(data, bucket, date) : await _deduct(data, bucket, date)
...
const _add = async (data, bucket, date) => {
	...
	for (const key in data.entries) data.entries[key].push(entry)
